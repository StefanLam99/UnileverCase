...
model {
        ...
        y ~ bernoulli_logit(x_beta[,2]);
}

generated quantities {
        real y_pred_insample[N];
        real y_pred_outsample[N_test];
        
        matrix[N, K] x_beta_train = x * beta';
        matrix[N_test, K] x_beta_test = x_test * beta';
        
        for (n in 1:N){
          y_pred_insample[n] = inv_logit(x_beta_train[n,2]');
        }
        
        
        for (n in 1:N_test){
                y_pred_outsample[n] = inv_logit(x_beta_test[n,2]');
        }
